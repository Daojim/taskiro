...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user!.id;
      const credentials =
        await googleCalendarService.refreshAccessToken(userId);
...
/**
 * Get sync history (last few sync operations)
 */
router.get(
  '/sync/history',
  authenticateToken,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // For now, we'll return basic sync status info
      // In a production app, you might want to store detailed sync history in a separate table
      const history = [];
      ...
      if (integration.lastSyncAt) {
        history.push({
          ...
          type: 'manual', // Could be 'manual' or 'automatic'
        });
      }
